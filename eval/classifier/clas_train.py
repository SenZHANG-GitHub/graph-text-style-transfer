...
flags.DEFINE_string("config", "config_train_yelp", "The config to use.")
flags.DEFINE_string("dataset", "yelp", "The dataset to train.")

FLAGS = flags.FLAGS

config = importlib.import_module(FLAGS.config)
if FLAGS.dataset not in FLAGS.config:
    raise ValueError('--dataset should be consistent with --config')

if os.path.isdir('ckpt_{}'.format(FLAGS.dataset)):
    shutil.rmtree('ckpt_{}'.format(FLAGS.dataset))
os.mkdir('ckpt_{}'.format(FLAGS.dataset))
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(config.num_epochs):
            ...
            if val_accu > best_val_accu:
                ...
                # e.g. ckpt_yelp/ckpt-1
                saver.save(sess, 'ckpt_{}/ckpt'.format(FLAGS.dataset), global_step=1)

if __name__ == '__main__':
    tf.logging.set_verbosity(tf.logging.INFO)
    tf.app.run(main=clas_train)
