...
__all__ = [
    "GraphMultiheadAttentionEncoder"
]

class GraphMultiheadAttentionEncoder(EncoderBase):
    """Graph Multihead Attention Encoder
    ...
    def _build(self, queries, memory, adj_masks, memory_attention_bias,
        ...
        Args:
            ...
            adj_masks: A 3d tensor with shape of [batch, length_key, length_key].
        ...
        with tf.variable_scope(self.variable_scope):
            ...
            # Q is computed from queries
            Q = self.Q_dense(queries)
            # both K and V are computed from memory
            # if memory is None, then use queries as memory
            ...
            #weights: [batch_size, num_heads, seq_length, memory_depth]
            ...
            #adj_masks: [batch_size, 1, seq_length, seq_length]
            adj_masks = tf.expand_dims(adj_masks, axis=1)
            weights = tf.multiply(weights, adj_masks)
