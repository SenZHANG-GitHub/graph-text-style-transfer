...
class GraphTextTransModel(object):
    ...
    def _build_model(self, inputs, vocab, gamma, lambda_g_graph, lambda_g_sentence):
        """Builds the model.
        """
        glove_embedding = tx.data.Embedding(
            vocab=vocab.token_to_id_map_py,
            hparams=self._hparams.glove
        )
        embedder = WordEmbedder(
            init_value=glove_embedding.word_vecs,
        ...
        gtt_connector = MLPTransformConnector(self._hparams.encoder.dim)
        pre_embedding_text_ids = tf.map_fn(fn=lambda x: gtt_connector(x), elems=(pre_embedding_text_ids), dtype=tf.float32, back_prop=True)
        ...
        g_vars = collect_trainable_variables(
            [embedder, encoder, label_connector, decoder, rephrase_encoder, rephrase_decoder, gtt_connector])
