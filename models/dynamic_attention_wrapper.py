...
class _BaseDynamicAttentionMechanism(AttentionMechanism):
  """A base AttentionMechanism class providing common functionality.

  Common functionality includes:
    1. Storing the query and memory layers.
    2. Let the memory to be dynamic that can be fed in during runing time
  ...
  def __init__(self,
    ...
    memory-related ops were moved to self.initialize_memory()
    ...
    self.check_inner_dims_defined = check_inner_dims_defined
  
  def initialize_memory(self, memory):
    with ops.name_scope(
        self.name, "BaseAttentionMechanismInit", nest.flatten(memory)):
      self._values = _prepare_memory(
          memory, self.memory_sequence_length,
          check_inner_dims_defined=self.check_inner_dims_defined)
      self._keys = (
...
class DynamicBahdanauAttention(_BaseDynamicAttentionMechanism):
  ...
  def __init__(self,
               ...
               score_mask_value=None,
               dtype=None,
               name="DynamicBahdanauAttention"):
    """Construct the Attention mechanism.

    memory was removed from __init__() of :class:_BaseDynamicAttentionMechanism
    ...
    super(DynamicBahdanauAttention, self).__init__(
  ...
  def __call__(self, query, state):
    ...
    Returns:
      alignments: Tensor of dtype matching `self.values` and shape
        ...
        `max_time`).
    """
    with variable_scope.variable_scope(None, "dynamic_bahdanau_attention", [query]):
...
class DynamicAttentionWrapper(rnn_cell_impl.RNNCell):
  ...
  def __init__(self,
    ...
    **NOTE** If you are using the `BeamSearchDecoder` with a cell wrapped in
    ...
    """
    super(DynamicAttentionWrapper, self).__init__(name=name)
